...
Original file is located at
    https://colab.research.google.com/drive/19LTzIwXvQoe_QmqoLNbX7UZJ_XojEf5O
...
for i in auto.tqdm(range(number_of_slices_to_try_to_generate)):
  ...
  try:  
    ...
    d1 = [int(y[2] / 1) for y in song[-1][-overlap_notes:]]
    ch1 = [int(y[3]) for y in song[-1][-overlap_notes:]]
    v1 = [int(y[5] / 1) for y in song[-1][-overlap_notes:]]
    
    tds1 = [int(abs(song[-1][-overlap_notes:][i-1][1]-song[-1][-overlap_notes:][i][1]) / 1) for i in range(1, len(song[-1][-overlap_notes:]))]
    if len(tds1) != 0: atds1 = int(sum(tds1) / len(tds1))
    
    for qp in quarter_pairs:

          p2 = [y[4] for y in qp[:overlap_notes]]
          d2 = [int(y[2] / 1) for y in qp[:overlap_notes]]
          v2 = [int(y[5] / 1) for y in qp[:overlap_notes]]
