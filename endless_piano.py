...
Original file is located at
    https://colab.research.google.com/drive/1UzzMdw7bGPl3RH_hSQt56u-KPlffLZzB
...
#@markdown NOTE: Generator DOES get stuck. So if nothing is being generated after 10-15 seconds, restart the generator.

randomize_dataset = False #@param {type:"boolean"}
self_conditioned_generation = True #@param {type:"boolean"}
...
desired_minimum_fuzzy_match_percentage = 100 #@param {type:"slider", min:0, max:100, step:1}
              ...
              if fuzz.ratio(c[1][1:], m[i+1][1:]) > desired_minimum_fuzzy_match_percentage and fuzz.ratio(c[0][1:], m[i+1][1:]) < desired_maximum_fuzzy_match_percentage:
                if fuzz.ratio(c[2][1:], m[i+2][1:]) > desired_minimum_fuzzy_match_percentage and fuzz.ratio(c[0][1:], m[i+2][1:]) < desired_maximum_fuzzy_match_percentage:
                  ...
                  song.extend(TMIDI.Tegridy_Timings_Converter(score, 
                                                              ...
                                                              max_delta_time=100,
                                                              mult = secrets.choice([0.1, 1.3, 1.4, 1.5]),
