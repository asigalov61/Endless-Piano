...
slices_match_type = "pitches_durations_and_velocities" #@param ["pitches_only", "pitches_and_durations", "pitches_and_beat", "pitches_durations_and_beat", "pitches_durations_and_velocities", "pitches_durations_velocities_and_beat", "pitches_durations_velocities_beat_and_channel"]
...
for i in auto.tqdm(range(number_of_slices_to_try_to_generate)):
  ...
  try:  
    ...
    d1 = [int(y[2] / 32) for y in song[-1][-overlap_notes:]]
    ch1 = [int(y[3]) for y in song[-1][-overlap_notes:]]
    v1 = [int(y[5] / 8) for y in song[-1][-overlap_notes:]]
    
    tds1 = [int(abs(song[-1][-overlap_notes:][i-1][1]-song[-1][-overlap_notes:][i][1]) / 32) for i in range(1, len(song[-1][-overlap_notes:]))]
    if len(tds1) != 0: atds1 = int(sum(tds1) / len(tds1))
    
    for qp in quarter_pairs:

          p2 = [y[4] for y in qp[:overlap_notes]]
          d2 = [int(y[2] / 32) for y in qp[:overlap_notes]]
          v2 = [int(y[5] / 8) for y in qp[:overlap_notes]]
          ch2 = [int(y[3]) for y in qp[:overlap_notes]]
          
          tds2 = [int(abs(qp[:overlap_notes][i-1][1]-qp[:overlap_notes][i][1]) / 32) for i in range(1, len(qp[:overlap_notes]))]
