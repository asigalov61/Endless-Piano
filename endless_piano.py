...
Original file is located at
    https://colab.research.google.com/drive/12EnnpfhH-IRmO0qLOThq2JydftkwdTBs
...
for qp in auto.tqdm(quarter_pairs1[0]):
  quarter_pairs.extend(TMIDI.Tegridy_Score_Slicer(qp, slices_length_in_miliseconds)[0])
print('Done!')
print('=' * 50)
...
#@markdown NOTE: Multipliers == 1 performs exact slices match, otherwise the mathcing is fuzzy based on provided multipliers.

number_of_slices_to_try_to_generate = 5 #@param {type:"slider", min:1, max:20, step:1}

slices_averages_multiplier = 0.2 #@param {type:"slider", min:0, max:1, step:0.05}
...
for i in auto.tqdm(range(number_of_slices_to_try_to_generate)):
  ...
  for s in signatures:
    ...
    if s1 == s2 and signatures.index(s) != signatures.index(sig):
      if quarter_pairs[signatures.index(s)] not in song:
        song.append(quarter_pairs[signatures.index(s)])
