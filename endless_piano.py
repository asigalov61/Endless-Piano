...
Original file is located at
    https://colab.research.google.com/drive/1T-M1pHje5OLPOBLAeYWgNQ3E6fY1r-tS

# Endless Piano (ver. 1.0)
...
desired_number_of_notes = 500 #@param {type:"slider", min:60, max:1000, step:1}
...
for i in range(len(chords_list)):
  ...
  new_song = chords_list[secrets.randbelow(len(chords_list))]
  # if new_song[0][4] == song[-1][4]: song.extend(new_song)
  song.extend(new_song) 
  for c in chords_list:
    if c[0][4] == song[-1][4]:
      d = secrets.choice([5, 4, 3, 2, 1])
      if int(c[0][2] / d) == int(song[-1][2] / d):
        if int(c[0][5] / d) == int(song[-1][5] / d):

            if secrets.randbelow(2): song.extend(c)
            l = (len(c) * -1)
