...
Original file is located at
    https://colab.research.google.com/drive/14i3G5Ny24kC1eBtYF40_CLaAe4gIC2L6
...
number_of_slices_to_try_to_generate = 20 #@param {type:"slider", min:1, max:100, step:1}
slices_fuzz_matching_threshold = 95 #@param {type:"slider", min:70, max:100, step:1}
...
for i in auto.tqdm(range(number_of_slices_to_try_to_generate)):
  try:
    for s in signatures:
      ...
      s1 = ' '.join([str(y) for y in sig][6:11])
      s2 = ' '.join([str(y) for y in s][6:11])
            
      if fuzz.ratio(s1, s2) >= slices_fuzz_matching_threshold and signatures.index(s) != signatures.index(sig):
